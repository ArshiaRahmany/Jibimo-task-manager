...
watch(
  () => props.payload,
  (newValue) => {
    Trello.title = newValue.title
    Trello.last_modified = newValue.last_modified
    Trello.containers = newValue.containers
    Trello.cards = newValue.cards
  },
  { immediate: true }
)
watch(
...
const cardList = (containerId) => {
  return Trello.cards.filter((card) => card.id_container === containerId)
}
...
const dropItem = (event, containerId) => {
  ...
  const item = Trello.cards.find((card) => card.id == id)
  item.id_container = containerId
  Trello.last_modified = new Date().toLocaleString('en-GB')
  store.dispatch('Trello/setTrello', Trello)
}
...
const deleteItem = (type) => {
  if (type === 'container') {
    Trello.containers = Trello.containers.filter(
...
const handleKanbanAction = (mode, type, containerId, payload) => {
  if (mode === 'add') {
    if (type === 'container') {
      const newContainer = {
        id:
          Trello.containers.length > 0
            ? [...Trello.containers].pop().id + 1
            : 1,
        name: newContainerTitle.value,
      }
      Trello.containers.push(newContainer)
      state.isAddingContainer = false
    } else {
      newCardData.id =
        Trello.cards.length > 0 ? [...Trello.cards].pop().id + 1 : 1
...
const handleEditCard = (type, selectedCard) => {
  if (type === 'change') {
    ...
    state.tempCards = Trello.cards.map((card) => ({ ...card }))
  } else if (type === 'save') {
...
<template>
  ...
  >
    <TransitionGroup name="list">
      <div
        v-for="container in Trello.containers"
        :key="container.id"
        class="mx-1"
      >
        <div
