...
const payload = computed(() => {
  return store.getters['Trello/getTrelloDatas']
})

onBeforeMount(async () => {
  const data = store.getters['Trello/getTrelloDatas']
  if (!data) {
    await axios.get('/sample-data.json').then(({ data }) => {
      store.dispatch('Trello/setTrello', data)
    })
  }
})
...
const handleEditTitle = (type) => {
  if (type === 'edit') {
    ...
    store.dispatch('Trello/setTrello', payload.value)
  } else {
    state.is_editing_title = false
